...
class TestProductionScenarios:
    ...
    def test_full_workflow_legacy_agent(self):
        ...
        # CRITICAL: Import INSIDE the patch context to avoid real API calls
    ...
    def test_large_dataset_handling(self):
        ...
        with patch('spotify_agent.spotify_client.SpotifyClient') as MockSpotifyClient, \
            ...
            agent = PodcastAgent(config)
            # Add preference to trigger episode processing
            agent.add_podcast_preference(PodcastPreference(topics=["technology"]))
            ...
            if "episodes" in result and result["episodes"]:
                ...
                # If no episodes, verify the message indicates success but no episodes added
                assert "Added 0 episodes to queue" in result["message"] or "No new relevant episodes found" in result["message"]